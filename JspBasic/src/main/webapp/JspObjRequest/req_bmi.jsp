<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    	...
    	담은 변수를 요청 파라미터라고 한다.
    	- 요청 파아미터는 URL 주소 뒤에 ?를 붙인 후에 [파라미터변수명=값]의 형식을 통해서
    	서버로 데이터를 전송
    	- 요청 파라미터를 여러 개 전달할 때는 & 기호를 사용하여 나열해서 전달.
     --%>
     
     <%
     	//클라이언트 쪽에서 전송된 요청 파라미터 값을 얻는 방법
     	//getParameter()의 리턴타입이 String.
     	//숫자가 전달되어도 타입이 문자열이기 때문에, 연산이 필요하다면 변환해서 쓰기.
