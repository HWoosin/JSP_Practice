...
//을 지정하면 서블릿을 등록하고 url mapping하는 과정을 대신할 수 있다.
//1.디렉토리 패턴: 디렉토리 형태로 서버의 해당 컴포넌트를 찾아서 실행하는 구조
// -> 정해진 url로만 서블릿에 요청을 보낼수가 있다.
//2.확장자 패턴: 확장자 형태로 서버의 해당 컴포넌트를 찾아서 실행하는 구조
// *.board -> 앞에 어떠한 경로가 존재하던지 .board 요청으로만 끝나면 해당 서블릿이 반응.

@WebServlet("*.board")//*. 모든것~
public class FrontController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String conPath = request.getContextPath(); // -> /JspBasic 라고 문자열이 들어가있음
		
		uri = uri.substring(conPath.length()+1, uri.lastIndexOf("."));// ->/JspBasic과 .board 사이의 값을 가져온다.
