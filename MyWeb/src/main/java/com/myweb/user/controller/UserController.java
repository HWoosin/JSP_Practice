...
public class UserController extends HttpServlet {
	...
	//요청이 무엇이냐에 따라 하나의 변수로 여러종류의 객체를 대입.
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch (uri) {
		case "joinPage":
			System.out.println("회원가입 페이지로 이동요청.");
		...
		case "join":
			...
			break;
		case "loginPage":
			System.out.println("로그인 페이지로 이동요청.");
		...
		case "myPage":
			...
			break;
		case "pwPage":
			System.out.println("비밀번호 변경 페이지로 이동요청.");
		...
		case "changePw":
			...
			break;
		case "modPage":
			System.out.println("회원정보 변경 페이지로 이동요청.");
			response.sendRedirect("user/user_update.jsp");
			break;
		case "update":
			System.out.println("회원정보 변경요청.");
			sv = new UpdateService();
			sv.execute(request, response);
			break;
		case "delPage":
			System.out.println("탈퇴 페이지로 이동요청.");
