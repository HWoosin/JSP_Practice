 ...
 * 비즈니스 로직이란, 외부 프로그램과 연계 작업등을 수행하는 로직.
 * 
 * VO(value object)
 * : 데이터베이스에서 조회된 값, 서버 프로그램에서 요청과 함께 얻은 값 등을
 * 포장에서 운반하는 값에 관련된 객체 이다.
 * VO 클래스는 자바빈 클래스로 생성함.
 * 
 * 자바빈 (java bean)
 * 웹 프로그래밍에서 어떠한 객체를 가지고 오기 위한 기법이며, 자바로 작성된 클래스를 일반적으로 부르는 호칭.
 * 자바빈을 설계하기 위한 규약이 존재.
 * 
 * 1. 은닉(캡슐화)을 구현해서 클래스를 제작한다. (private, getter/setter)
 * 2. getter 메서드는 매개변수가 존재하지 않아야 한다.
 * 3. 생성자는, 매개값을 받지 않는 생성자(필수), 매개값을 받는 생성자(선택)가 존재.
