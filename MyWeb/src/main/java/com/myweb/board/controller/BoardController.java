...
public class BoardController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "list":
			...
			/*
			 * request객체에 list를 담아서 전달하려 하는데, sendRedirect를 사용하면 응답이바로 나가면서 request객체가 소멸해 버린다.
			 * 여기서는 forward방식을 사용해서 request를 원하는 jsp 파일로 전달해서 그쪽에서 응답이 나갈 수 있도록 처리해야한다.
			 */
//			response.sendRedirect("board/board_list.jsp");
		...
		case "regist":
			...
			sv.execute(request,response);
			/*
			 * 왜 board_list.jsp로 바로 리다이렉트를 하면 안되냐면, board_list.jsp에는 데이터베이스로부터 전체 글 목록을
			 * 가져오는 로직을 작성하지 않을 것이기 때문.(jsp는 단순히 보여지는 역할만 수행)
