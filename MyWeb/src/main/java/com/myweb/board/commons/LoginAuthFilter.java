...
@WebFilter(
		...
		urlPatterns = {"/write.board", "/modify.board", "/myPage.user", "/modPage.user"},
		servletNames = {"basic"}
		)
public class LoginAuthFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		/*
		 - 필터의 핵심 메서드! doFilter()는 클라이언트의 요청이 있을때마다 실행된다.
		 - request객체와 response 객체를 넘겨주기 때문에 이를 가지고 요청과 응답을 조작할 수 있다.
		 - 그리고 FilterChain을 통해 조작 이후 요청을 원래 목적지인 서블릿으로 전달할 수 있다.
		 */
		System.out.println("doFilter 메서드 발동!");
		
		//우리가 평소에 사용하던 타입은 HttpServletRequest이다.
		//부모 타입의 ServletRequest를 자식타입으로 끌어 내리면 사용이 가능.
		...
		if(session.getAttribute("user") == null) {
			...
			out.flush();
			out.close();
			return; // 로그인 유효성을 통과하지 못했기 때문에 여기서 요정은 강제종료.
		}
	...
	public void init(FilterConfig fConfig) throws ServletException {
		...
		//이 때 객체가 생성되면서 최초 한번 호출되는 메서드이다.
		//필터에서 처리 시 필요한 객체 등을 초기화(JDBC 관련 설정) 하는 데 사용한다.
